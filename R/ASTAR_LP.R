...
ASTAR_LP <- function(Pi,X,M){
  ...
  for(order in 1:J){open[order,J+1] <- totalcostheuristic_MB_LP(Q,Pi,X,M,order)}
  ...
  while(continue){
    ...
    for(ind in 1:nrow(open_new)){open_new[ind,J+1] <- totalcostheuristic_MB_LP(Q,Pi,X,M,na.exclude(open_new[ind,1:J]))}
