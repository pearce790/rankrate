...
GreedyLocal <- function(rankings,ratings,M){
  ...
  which_min <- 1:nrow(result)

  # CONTINUE ESTIMATION WITH LOCAL SEARCH
  continue <- TRUE
  while(continue){
    for(row in 1:nrow(result)){
      nearby_rankings <-  matrix(unlist(lapply(1:(J-1),function(j){
        new <- result[row,]
        new[j:(j+1)] <- result[row,(j+1):j]
        new
      })),byrow=T,ncol=J)
      result <- rbind(result,nearby_rankings)
    }
    ...
    num_nodes <- num_nodes + nrow(results)
    which_minlocal <- which(results[,J+2]==min(results[,J+2]))
    if(any(which_min == which_minlocal)){
      continue <- FALSE
